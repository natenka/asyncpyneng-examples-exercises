...
async def telnet_scan(devices):
    ...
    async for status, msg in check:
        if status:
            print(f"Telnet. Подключение успешно: {msg}")
        else:
            print(f"Telnet. Не удалось подключиться: {msg}")


async def main():
    devices = ["192.168.100.1", "192.168.100.2", "192.168.100.3", "192.168.100.15"]
    await asyncio.gather(ssh_scan(devices), telnet_scan(devices))
